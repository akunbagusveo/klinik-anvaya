const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbwtKbuvQhrLcVRaoo33z6MmV6oMRBK_ZGTkPFyNH5xNgAxG1j3H5GJAFuGW9WzzvKhS/exec"; 
    ...
    // 1. Variabel penampung kamus dari Google Sheets
    let kamusKoreksiDinamis = {};
    let currentPage = 1;
    const rowsPerPage = 10; // Jumlah baris yang ingin ditampilkan per halaman
    let dataKamusArray = []; // Array untuk menyimpan data siap render
    ...
    // 2. Fungsi untuk menarik data kamus saat halaman web dibuka
    function muatKamusDikte() {
        ...
        if (typeof tampilkanLoading === "function") tampilkanLoading("⏳ Memuat data Kamus Dikte dari Server...");
        ...
        .then(res => {
            ...
            if (res.result === "success") {
                ...
                // 🔥 THE MAGIC TRICK: Balik urutan array agar yang TERBARU (Paling Bawah di Sheet) muncul di ATAS (Halaman 1)
                arrayUntukTabel.reverse();

                kamusKoreksiDinamis = objekKoreksiBersih; 
                dataKamusArray = arrayUntukTabel;
                
                currentPage = 1; // Selalu kembali ke Halaman 1 saat di-refresh
                
                if (bodyKamus && typeof renderKamusTable === "function") {
                    renderKamusTable(); 
                }
            } else {
                if (bodyKamus) {
    ...
    // 2. Update fungsi renderKamusTable
    function renderKamusTable() {
        ...
        if (dataKamusArray.length === 0) {
            ...
            paginationDiv.style.display = 'none'; // Sembunyikan pagination jika kosong
            return;
        }

        paginationDiv.style.display = 'block'; // Tampilkan pagination

        // 🔥 BARU: Hitung potongan data berdasarkan halaman saat ini
        const startIndex = (currentPage - 1) * rowsPerPage;
        const endIndex = startIndex + rowsPerPage;
        const paginatedItems = dataKamusArray.slice(startIndex, endIndex);
        
        // Loop hanya pada data yang sudah dipotong (maksimal 10)
        ...
        // 🔥 BARU: Update informasi teks Halaman dan status Tombol
    ...
    // 3. 🔥 BARU: Fungsi untuk mengontrol tombol Next/Prev
    function ubahHalaman(direction) {
        const totalPages = Math.ceil(dataKamusArray.length / rowsPerPage);
        currentPage += direction;
        
        // Validasi pencegahan error (meski tombol sudah di-disable)
        if (currentPage < 1) currentPage = 1;
        if (currentPage > totalPages) currentPage = totalPages;
        
        renderKamusTable(); // Gambar ulang tabel dengan data halaman baru
    }

// Catatan: Fungsi tambahKamus() dan hapusKamus() Anda tidak perlu diubah, biarkan seperti semula.

    // 3. Otomatis jalankan fungsi saat web selesai dimuat
    window.addEventListener('load', muatKamusDikte);
    ...
    // Fungsi aksi tombol "Tambah Data" ke Google Sheets via Fetch
    function tambahKamus() {
        ...
        // =====================================================================
        // 🔥 LAPIS 1: SATPAM PENCEGAH DUPLIKAT (FRONTEND)
        // Mengecek dari memori browser secara instan tanpa perlu ke server
        // =====================================================================
        const salahLower = salah.toLowerCase();
        // Kita manfaatkan variabel kamusKoreksiDinamis yang sudah dibuat oleh muatKamusDikte
        if (typeof kamusKoreksiDinamis !== "undefined" && kamusKoreksiDinamis[salahLower]) {
            const kataLama = kamusKoreksiDinamis[salahLower];
            alert(`🚫 KATA SUDAH TERDAFTAR!\n\nKata salah "${salah}" sudah ada di dalam kamus dengan perbaikan menjadi "${kataLama}".\n\nJika ingin mengubahnya, silakan hapus data yang lama terlebih dahulu di tabel bawah.`);
            return; // Hentikan proses secara instan!
        }
        // =====================================================================
        
        if (typeof tampilkanLoading === "function") tampilkanLoading("⏳ Menyimpan kata baru ke Kamus...");
        ...
        .then(res => {
            if (res.result === "success") {
                ...
                if (typeof sembunyikanLoading === "function") sembunyikanLoading();
                alert("❌ Gagal menyimpan kata: " + res.message); // Akan memunculkan pesan dari Backend jika duplikat lolos
            }
        })
        .catch(err => {
            if (typeof sembunyikanLoading === "function") sembunyikanLoading();
    ...
    // Fungsi aksi tombol "Hapus" dari Google Sheets via Fetch
    function hapusKamus(rowIndex) {
        if (!confirm("Apakah Anda yakin ingin menghapus baris kata ini?")) return;
        
        // =====================================================================
        // 🔥 PANGGIL LAYAR LOADING DI SINI (Saat menghapus)
        // =====================================================================
        if (typeof tampilkanLoading === "function") tampilkanLoading("⏳ Menghapus kata dari database...");
        ...
        .then(res => res.json())
        .then(res => {
            // 🔥 MATIKAN LAYAR LOADING
            if (typeof sembunyikanLoading === "function") sembunyikanLoading();

            if (res.result === "success") {
                alert("Kata berhasil dihapus!");
                muatKamusDikte(); // Muat ulang tabel
            }
        })
        .catch(err => {
            // 🔥 MATIKAN LAYAR LOADING
    ...
    // Fungsi Voice-to-Text (Dikte)
    // 🔥 FUNGSI DIKTE SUARA (RE-USE & UPGRADE UNTUK MAC CHROME)
    // 🔥 FUNGSI DIKTE SUARA CONTINUOUS (Mode Tahan Jeda & Bisa Di-Stop)
    let pengenalSuaraAktif = null; // Menyimpan status mic

    // 🔥 SOLUSI BIJAK: Kita tidak perlu lagi menulis "let pengenalSuaraAktif = null;" secara terpisah di luar fungsi.
    // Kita langsung kelola statusnya secara dinamis menggunakan objek global browser (window.pengenalSuaraAktif)

    function mulaiDikte(targetId) {
        ...
        recognition.onresult = function(event) {
            ...
            // 2. INTEGRASI KAMUS DIKTE MULTI-FORMAT
            if (typeof kamusKoreksiDinamis !== 'undefined' && kamusKoreksiDinamis !== null) {
                let daftarKoreksiAman = [];
                if (Array.isArray(kamusKoreksiDinamis)) {
                    kamusKoreksiDinamis.forEach(item => {
                        if (Array.isArray(item) && item.length >= 2) {
                            if (item[0].toString().toLowerCase().trim() !== "kata salah") {
                                daftarKoreksiAman.push({ salah: item[0], benar: item[1] });
                            }
                        } else if (item && item.salah && item.benar) {
                            daftarKoreksiAman.push({ salah: item.salah, benar: item.benar });
                        }
                    });
                } else {
                    for (const [salah, benar] of Object.entries(kamusKoreksiDinamis)) {
                        daftarKoreksiAman.push({ salah: salah, benar: benar });
                    }
                }

                daftarKoreksiAman.forEach(koreksi => {
                    let kataSalahBersih = koreksi.salah.toString().trim().replace(/[\s\u00A0]+/g, " ");
                    let kataBenarBersih = koreksi.benar.toString().trim();
                    if (kataSalahBersih !== "") {
                        const polaRegex = kataSalahBersih.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').replace(/ /g, '[\\s\\u00A0]+');
                        const regexAman = new RegExp(polaRegex, "gi");
                        textNormal = textNormal.replace(regexAman, kataBenarBersih);
                    }
                });
            }
            ...
            // 3. MERAPIKAN TANDA BACA
            hasilText = hasilText.replace(/\s+,/g, ",");
            hasilText = hasilText.replace(/\s+\./g, ".");
            hasilText = hasilText.replace(/,\s*\./g, ",");
            hasilText = hasilText.replace(/\.\s*,/g, ",");
            hasilText = hasilText.replace(/\.{2,}/g, ".");

            // 4. KUNCI ALUR SEBAGAI PARAGRAF MENGALIR BIASA DI FORM
            if (hasilText) {
                ...
                if (harusKapital) {
                    hasilText = hasilText.charAt(0).toUpperCase() + hasilText.slice(1);
                }
                
                // Masukkan teks ke dalam kotak input
                targetInput.value += spasiPemisah + hasilText;
                targetInput.value = targetInput.value.replace(/\s{2,}/g, " ");
                
                // =====================================================================
                // 🔥 THE MAGIC FIX: Tambahkan { bubbles: true }
                // Agar sistem Dropdown ICD-10 menyadari ada teks yang masuk via suara
                // =====================================================================
        ...
        recognition.onerror = function(event) {
            ...
            if(event.error === 'not-allowed') {
                alert("Izin mikrofon diblokir. Silakan aktifkan izin mikrofon pada browser Anda.");
            }
            window.pengenalSuaraAktif = null;
            btnMicrophone.innerText = originalIcon;
        };
